...
class AuthController {
    constructor() {
        this.validateLicense = async (req, res) => {
            try {
                const { license, ip, script_name } = req.body;
        ...
        this.createLicense = async (req, res) => {
            try {
                ...
                const licenseKey = await this.authService.createLicense(name.trim(), script_name.trim(), authorized_ips.map((ip) => ip.trim()), expiresAt);
                res.status(201).json({
                    success: true,
        ...
        this.updateLicense = async (req, res) => {
            try {
                ...
                const updated = await this.authService.updateLicense(license_key.trim().toUpperCase(), updates);
                if (updated) {
                    res.json({
                        success: true,
        ...
        this.updateIps = async (req, res) => {
            try {
                ...
                const updated = await this.authService.updateAuthorizedIps(license_key.trim().toUpperCase(), authorized_ips.map((ip) => ip.trim()));
                if (updated) {
                    res.json({
                        success: true,
        ...
        this.getAllLicenses = async (req, res) => {
            try {
                const { page, limit, status, script_name } = req.query;
                const pageNumber = page ? parseInt(page, 10) : 1;
                const limitNumber = limit ? parseInt(limit, 10) : 10;
                const statusFilter = status;
                const scriptFilter = script_name;
                const result = await this.authService.getAllLicenses(pageNumber, limitNumber, statusFilter, scriptFilter);
                res.json({
                    success: true,
        ...
        this.getAllLogs = async (req, res) => {
            try {
                const { page, limit, license_id, access_type, date_from, date_to, script_name } = req.query;
                const pageNumber = page ? parseInt(page, 10) : 1;
                const limitNumber = limit ? parseInt(limit, 10) : 10;
                const filters = {
                    ...
                    date_from: date_from ? new Date(date_from) : new Date(0),
                    date_to: date_to ? new Date(date_to) : new Date(),
                };
                const result = await this.authService.getAllLogs(pageNumber, limitNumber, filters);
                res.json({
                    success: true,
        ...
        this.getAvailableScripts = async (_req, res) => {
            try {
        ...
        this.deleteLicense = async (req, res) => {
            try {
                ...
                const deleted = await this.authService.deleteLicense(license_key.trim().toUpperCase());
                if (deleted) {
                    res.json({
                        success: true,
